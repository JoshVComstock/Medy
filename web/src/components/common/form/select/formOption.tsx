...
const FormOption = ({ children, value, disabled = false, tab = 0 }: Props) => {
  ...
  const classNames = [
    "pr-2 py-2 outline-none cursor-pointer disabled:bg-gray-50 disabled:text-black/20 disabled:cursor-auto flex gap-2 dark:bg-gray-500 dark:text-white",
  ];
  if (selected) {
    classNames.push("bg-primary-500 dark:bg-gray-800");
  } else {
    classNames.push("hover:bg-gray-100 focus:bg-gray-100 dark:hover:bg-gray-800");
  }
  const PClassNames = [
